...
class LlamaManagerApp:
    ...
    def _add_entry_shortcuts(self, widget) -> None:
        ...
        # We return 'break' to prevent the default class-level bindings from
        # triggering a second time.
        widget.bind('<Control-v>', lambda e: self._generate_virtual_event(widget, '<<Paste>>'))
        widget.bind('<Control-V>', lambda e: self._generate_virtual_event(widget, '<<Paste>>'))
        widget.bind('<Control-c>', lambda e: self._generate_virtual_event(widget, '<<Copy>>'))
        widget.bind('<Control-C>', lambda e: self._generate_virtual_event(widget, '<<Copy>>'))
        widget.bind('<Control-x>', lambda e: self._generate_virtual_event(widget, '<<Cut>>'))
        widget.bind('<Control-X>', lambda e: self._generate_virtual_event(widget, '<<Cut>>'))

    def _generate_virtual_event(self, widget, event_name) -> str:
    ...
    def update_all(self) -> None:
        ...
        try:
            ...
            messagebox.showerror('Error', f'Failed to save models.ini:\n{e}')
    ...
    def launch_llama(self) -> None:
        ini_path = self.manager.config.get('models_ini_path', '')
        if not ini_path:
            messagebox.showerror('Error', 'Models INI path is not set in Settings.\n\nLlama server requires a models.ini file to load your models.')
            return
